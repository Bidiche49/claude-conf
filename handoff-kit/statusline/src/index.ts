...
let getUsageLimits: ((options?: unknown) => Promise<unknown>) | null = null;
let normalizeResetsAt: ((resetsAt: string) => string) | null = null;
let getPeriodCost: ((periodId: string) => number) | null = null;
let getTodayCostV2: (() => number) | null = null;
let saveSessionV2:
	| ((input: unknown, resetsAt?: string) => Promise<void>)
	| null = null;
...
async function main() {
	try {
		...
		// Write context percentage to file for hooks to read (per session)
		const contextDir = join(process.env.HOME || "", ".claude", "context-data");
		await Bun.write(join(contextDir, ".gitkeep"), ""); // Ensure dir exists
		const contextPercentageFile = join(
			contextDir,
			`${input.session_id}.txt`,
		);
		await writeFile(contextPercentageFile, String(contextPercentage ?? 0));
